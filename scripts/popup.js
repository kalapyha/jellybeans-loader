(function (ls) {
  ...
  function initializePopup() {
    ...
    if (ls.show_url_filter === "true") {
      ...
      $("#filter_url_mode_input")
        .val(ls.filter_url_mode)
        .on("change", function () {
    ...
    if (
      ...
      var initializeFilter = function (dimension) {
        ...
        $("#image_" + dimension + "_filter_min_checkbox")
          .prop(
            "checked",
            ls["filter_min_" + dimension + "_enabled"] === "true"
          )
          .on("change", function () {
            toggleDimensionFilter(
              "image_" + dimension + "_filter_min",
        ...
        $("#image_" + dimension + "_filter_max_checkbox")
          .prop(
            "checked",
            ls["filter_max_" + dimension + "_enabled"] === "true"
          )
          .on("change", function () {
            toggleDimensionFilter(
              "image_" + dimension + "_filter_max",
  ...
  function initializeStyles() {
    ...
    $("#only_images_from_links_container").toggle(
  ...
  function filterImages() {
    ...
    timeoutID = setTimeout(function () {
      ...
      if (
        ...
        if (cached_images < allImages.length) {
          for (var i = cached_images; i < allImages.length; i++) {
            ...
            images_cache.append(
              $('<img src="' + encodeURI(allImages[i]) + '" />').on(
                "load",
                filterImages
              )
            );
      ...
      if (
        ...
        visibleImages = visibleImages.filter(function (url) {
          var image = images_cache.children(
  ...
  function displayImages() {
    ...
    var toggle_all_checkbox_row =
      '<tr><th align="left" colspan="' +
      ls.columns +
      '"><label><input type="checkbox" id="toggle_all_checkbox" />Select all (' +
    ...
    for (var rowIndex = 0; rowIndex < rows; rowIndex++) {
      if (
        ...
        for (var columnIndex = 0; columnIndex < columns; columnIndex++) {
          ...
          if (show_image_url) {
            tools_row.append(
              '<td><input type="text" class="image_url_textbox" value="' +
                visibleImages[index] +
                '" readonly /></td>'
            );
          }
          ...
          if (show_open_image_button) {
            tools_row.append(
              '<td class="open_image_button" data-url="' +
                visibleImages[index] +
                '" title="Open in new tab">&nbsp;</td>'
            );
          }

          if (show_download_image_button) {
            tools_row.append(
              '<td class="download_image_button" data-url="' +
                visibleImages[index] +
                '" title="Download">&nbsp;</td>'
            );
          }
      ...
      for (var columnIndex = 0; columnIndex < columns; columnIndex++) {
        ...
        var image =
          ...
          "px; width: " +
          columnWidth +
          '; vertical-align: top;"><img id="image' +
          index +
          '" src="' +
          visibleImages[index] +
          '" /></td>';
  ...
  function flashDownloadingNotification(imageCount) {
    ...
    var downloading_notification = $(
      '<div class="success">Downloading ' +
        imageCount +
        " image" +
        (imageCount > 1 ? "s" : "") +
        "...</div>"
